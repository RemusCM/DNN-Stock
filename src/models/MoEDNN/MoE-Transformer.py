...
#model_name = 'MoE-Transformer-100'
model_name = 'MoE-Transformer-750'
...
for index,val in enumerate(list_of_stocks):
    ...
    class GatingTransformer(nn.Module):
        def __init__(self, input_dim, num_experts, d_model, nhead, num_layers, dim_feedforward):
    ...
    # MixtureOfExperts Class with Transformer-based Gating
    class MixtureOfExperts(nn.Module):
        def __init__(self, input_dim, output_dim, num_experts=2, d_model=64, nhead=4, num_layers=4, dim_feedforward=64):
            super(MixtureOfExperts, self).__init__()
            self.experts = nn.ModuleList([Expert(input_dim, output_dim, d_model, nhead, num_layers, dim_feedforward) for _ in range(num_experts)])
            
            # Transformer-based Gating mechanism
            self.gate = GatingTransformer(input_dim, num_experts, d_model=32, nhead=2, num_layers=2, dim_feedforward=32)

        def forward(self, src, tgt):
            # Get the gating weights for each expert
            gate_outputs = self.gate(src)
            
            # Get the output from each expert
            expert_outputs = [expert(src, tgt) for expert in self.experts]
            
            # Compute the weighted sum of expert outputs
            final_output = sum(gate_outputs[:, i].unsqueeze(1) * expert_outputs[i] for i in range(len(self.experts)))
            
            return final_output
        
        
    model = MixtureOfExperts(
        input_dim=22, num_experts=2, output_dim=2, d_model=64, nhead=4, num_layers=4, dim_feedforward=64
    )
